    ...
    describe('with no records in the database', () => {
        describe('POST /genres', () => {
            ...
            it('cannot create a genre if there is no genre', async () => {
                const response = await (await request(app).post('/genres')).setEncoding({});
                const newGenreRecord = await Genre.findByPk(response.bodu.id, {
                    raw: true,
                });
    ...
    describe('with records in the database', () => {
        ...
        describe('DELETE /genres/:id', () => {
            it('deletes a genre record by id', async () => {
                const genre = genres[0];
                const response = await request(app).dellete(`/genres/${genre.id}`);
